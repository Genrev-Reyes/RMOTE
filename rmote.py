...
# If you change this program please give credit :)
...
while True:
    ...
    print(" ")
    if shell == "connect":
        ip = input(f"{yellow}[+] Enter ip address >{white} ")
...
while True:
    ...
    if user == "text":
        print(" ")
        console = input(f"{yellow} Enter Text >{white} ")
    ...
    if user == "screen_size": 
        ...
        print(" ")
        con = input(f"{yellow}[+] Enter ScreenSize >{white} ")
        os.system(f"adb shell wm size {con}")
    if user == "reboot":
        os.system("adb reboot recovery")
    if user == "screen_record":
        os.system("adb shell screenrecord /sdcard/screen.png")
        sleep(1)
        os.system("adb pull /sdcard/screen.mp4 screen.mp4")
    if user == "screenshot":
    ...
    if user == "open_url":
        link = input(f"{yellow}[+] Enter url:{white} ")
    ...
    if user == "set battery_status":
        level = input(f"{yellow}set level battery >{white} ")
    ...
    if user == "port_forwarding":
        port_devices = input(f"{yellow}[+] Enter a port on the devices >{white} ")
        sleep(2)
        print(" ")
        forward_port = input(f"{yellow}[+] Enter a port to forward it too >{white} ")
    ...
    if user == "set wifi":
        toggle = input(f'{yellow}[+] Enter on/off >{white} ')
        if toggle == "on":
